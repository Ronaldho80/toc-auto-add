...
fn get_file_titles(file_content: &str) -> Vec<Title> {
    ...
    for line in file_content.lines() {
        ...
        if first_level_title_re.is_match(line) {
            ...
            let title_string: &str =
                first_level_title_re.captures(line).unwrap().get(1).map_or("", |m| m.as_str());
            println!("First level title: {}", title_string);
            let t = Title {
                title: String::from(title_string),
                sub_titles: vec![],
            };
            file_titles.push(t);
            continue;
        ...
        if second_level_title_re.is_match(line) {
            ...
            let title_string: &str = second_level_title_re.captures(line).unwrap().get(1).map_or("", |m| m.as_str());
...
fn add_or_replace_toc(file_content: &str, toc: &str) -> String {
    let title_area_re =
        Regex::new(r"(?s)(?P<head><!-- toc-begin -->\n)(?P<body>.+)(?P<tail><!-- toc-end -->)")
            .unwrap();

    // has toc already, so update the toc
    if title_area_re.is_match(file_content) {
        let title_area_string: &str = title_area_re.captures(file_content).unwrap().get(2).map_or("", |m| m.as_str());
        ...
        return replaced_file_content.into_owned();
    }
    ...
    if more_comment_re.is_match(file_content) {
        let more_comment_string: &str =
            more_comment_re.captures(file_content).unwrap().get(0).map_or("", |m| m.as_str());
